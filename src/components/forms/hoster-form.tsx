...
interface HosterFormModalProps {
    isOpen: boolean;
    hoster?: HosterForm | null;
    onClose: () => void;
    callback: (reponse: any) => void;
    getErrorField: (field: string) => any
}

const HosterFormModal: React.FC<HosterFormModalProps> = ({ isOpen, hoster, onClose, getErrorField, callback }) => {
    ...
    useEffect(() => {
        if (hoster) {
            setFullname(hoster.fullname);
            setEmail(hoster.email);
        }
    }, [hoster]);
    ...
    return (<>
        <FormModal isOpen={isOpen} onClose={onClose}>
            <h2 className="text-xl font-semibold mb-4 text-slate-700 flex items-center">
                ...
                <span className='ml-2'>Hoster Information</span>
            </h2>
            ...
            <div className="flex items-center justify-between mt-4">
                <FormButton onClick={handleSeller} text={"Confirm & Submit"} isLoading={false} type="submit" className="mt-5" />
            </div>
        </FormModal>
    </>);
}

export default HosterFormModal;
